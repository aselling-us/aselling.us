...
`scripts/fetch-media.mjs` (`npm run fetch:media`) snapshots Goodreads shelf RSS (user `192886223`, shelves `read` + `currently-reading` + `to-read`), Letterboxd (user `aselling`), and GitHub (user `aselling-us`) into `src/data/goodreads.json` / `src/data/letterboxd.json` / `src/data/github.json`. Like the geocode cache, the snapshots are committed so a local build needs no network. `fetchedAt` is only bumped when feed content actually changes — the pages render it in a "most recent update" line, and it keeps the refresh workflow from committing no-op changes. `.github/workflows/refresh-media.yml` re-runs the fetch daily, commits if changed, and dispatches `deploy.yml` (a push from `GITHUB_TOKEN` won't trigger it otherwise). `deploy.yml`'s own `build` job *also* re-runs `fetch:media` right before `npm run build`, on every deploy (regardless of what triggered it) — so the live site always bakes in whatever's currently freshest rather than waiting for the next daily refresh, while `refresh-media.yml` remains the thing that commits a snapshot back to git. That step is `continue-on-error: true`: a flaky feed shouldn't block a real code deploy, and on failure the build just falls back to whatever `src/data/*.json` was already checked out.
...
`scripts/remark-footnote-titles.mjs` (wired in `astro.config.mjs`) rewrites links inside footnote definitions to use the fetched page `<title>` as link text. Titles are cached in `.cache/footnote-titles.json` — unlike the rest of `.cache/`, this one file is committed (carved out in `.gitignore`) like the geocode cache and media snapshots, because sites like YouTube serve GitHub Actions' shared IPs a stripped page with a blank `<title>` instead of erroring, so a fresh-every-build fetch is unreliable; a title is only cached once it comes back non-empty, and an empty/failed fetch falls back to the raw URL without poisoning the cache. Reddit links are left as raw URLs (Cloudflare blocks the fetch).
